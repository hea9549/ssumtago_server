...
class UsersController < ApplicationController
  ...
  def login
    ...
    if params[:joinType] == "facebook"
      ...
      begin @fb_info = @fb_user.verify
        ...
        begin @user = User.where(joinType: "facebook").find_by(email: params[:email])
  ...
  def create
    ...
    # joinType이 facebook인지 확인
    elsif params[:joinType] == "facebook"
      logger.info "[LINE:#{__LINE__}] joinType값 facebook 확인, 페이스북 토큰값 확인 중..."
      @fb_user = CheckFbToken.new(params[:password])
      # 페이스북 토큰이 맞는지 인증
      begin @fb_info = @fb_user.verify
        ...
        if @user.persisted?
          logger.error "[LINE:#{__LINE__}] email값 중복 / 통신종료"
          @error = {msg: "이미 가입된 회원입니다.", code:"400", time:Time.now}
          render json: @error, status: :bad_request
        # 페이스북 신규 회원일때
        else
    ...
    # joinType이 email이 아니라면 에러
      ...
      @error = {msg: "올바른 joinType 값을 넣어주세요! (email / facebook)", code:"400", time:Time.now}
      render json: @error, status: :bad_request
    end
  end
  ...
  def facebookCheck
    ...
    begin @fb_info = @fb_user.verify
      ...
      # 페이스북 신규 회원일때
        ...
        logger.info "[LINE:#{__LINE__}] 미가입 회원 확인 / 통신종료"
        @message = {msg:"페이스북 미가입 회원 입니다.", code:"204", time:Time.now}
